...
def get_top_tracks(offset):
    ...
    # CH:
    return track_list # how else will the outside get access to the track_list?  You sho
...
def get_audio_features(track_list):
    for track in track_list:
        ...
        try:
            # CH I'm guessing if this fails all is lost?
            track_audio_features = sp.audio_features(tracks=track['track_id'])[0] # only grab 0?
        except:
            track['audio_features'] = "No audio features for this track."
            continue

        # CH this is not recommended b/c it might bail out on the 3. line even if the rest
        # of lines would have worked.  Use this:
        # track['acousticness'] = track_audio_features.get('acousticness', None)  
        # This will return None if the key is not found
        # That means you will have to later test for these None values but at least you will
        # get all the data you can get.

        track['acousticness'] = track_audio_features.get('acousticness', None)
        track['danceability'] = track_audio_features.get('danceability', None)
        track['energy'] = track_audio_features.get('energy', None)
        track['instrumentalness'] = track_audio_features.get('instrumentalness', None)
        track['liveness'] = track_audio_features.get('liveness', None)
        track['speechiness'] = track_audio_features.get('speechiness', None)
        track['valence'] = track_audio_features.get('valence', None)

        none_count = sum(track[key] is None for key in ['acousticness', 'danceability', 'energy', 
                                                       'instrumentalness', 'liveness', 'speechiness', 'valence'])
        
        track['audio_features'] = f"{7 - none_count} of 7 audio features available for this track" 
        
    # CH: yes, mutable objects (lists, dicts) are passed by reference and can be changed in the function
    # but it's a good practice to return it anyway so it's clear that something goes in and something
    # changed comes out
    return track_list
...
def get_new_tracks(track_list):
    # CH: No! bad puppy :) global track_list # needed to use the global keyword because the function didn't recognize the global track_list object below
    ...
    return track_list #  CH returns modified track list

    
# CH: It's not recommended to use global variables.  Yes it's true that the function
# can read the global var track_list but you should pass it as an argument to the function
# In general if you should always have all your function inputs come as arguments and any
...
def genre_score_deduction(genre_input):
    ...
    for track in track_list:
        ...
        if any(genre in track['genres'] for genre in input_genres_list): # CH no need for outer () but still pretty clever :)
    ...
    # CH
    return track_list # return the modified track list



# Deduce the track score based on feature input
def feature_score_deduction(track_list, input_value, feature): # I am passing the feature name as an argument so I don't need to construct 7 functions for each audio feature
    for track in track_list:
...
for i in range(2): # This fires only twice to mitigate rate limiting. The real application will fire get_top_tracks 
    track_list = get_top_tracks(looper_offset) #CH
    looper_offset += 50
...
# CH please break your comments into multiple lines.  It's hard to read when they are so long.
...
track_list = set_artist_genres(track_list) # Genres are properties of artists, not tracks. This function pulls genres from artists and parses the genres to each track, which isn't logically perfect, but is a feature I think will be useful to users.
...
track_list = genre_score_deduction(genre_input)
...
track_list = feature_score_deduction(track_list, acousticness_input_value, feature='acousticness')
track_list = feature_score_deduction(track_list, danceability_input_value, feature='danceability')
track_list = feature_score_deduction(track_list, energy_input_value, feature='energy')
track_list = feature_score_deduction(track_list, instrumentalness_input_value, feature='instrumentalness')
track_list = feature_score_deduction(track_list, liveness_input_value, feature='liveness')
track_list = feature_score_deduction(track_list, speechiness_input_value, feature='speechiness')
track_list = feature_score_deduction(track_list, valence_input_value, feature='valence')
...
if create_new_playlist_input == 'y':
    new_playlist = create_new_playlist(playlist) # Create a new playlist with the top 30 displayed in this app
else:
